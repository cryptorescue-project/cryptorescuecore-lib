...
/**
 * Retrieve a hexa string that can be used with cryptorescued's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more CryptoRescue coins than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of cryptorescued's `listunspent` command,
 * and a slightly fancier format recognized by cryptorescuecore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a cryptorescuecore Address object. The
 * same is true for `script`, which can be a string or a cryptorescuecore Script.
 *
 ...
 *
 * // From a pay to public key hash output from cryptorescued's listunspent
 ...
 * CheckTransaction in CryptoRescue.
 */
...
/**
 * Analogous to cryptorescued's IsCoinBase function in transaction.h
 */
