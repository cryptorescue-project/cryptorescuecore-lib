...
We're working hard to make *cryptorescuecore* the most powerful JavaScript library for working with CryptoRescue. Our goal is to have *cryptorescuecore* be a library that can be used by anyone interested in CryptoRescue, and to level expertise differences with great design and documentation.
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of cryptorescuecore methods. For example, calling `JSON.stringify` on an cryptorescuecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
```sh
git remote add cryptorescue-project git@github.com:cryptorescue-project/cryptorescuecore.git
git pull --rebase cryptorescue-project master
```
...
[github.com/cryptorescue-project/cryptorescuecore/issues](https://github.com/cryptorescue-project/cryptorescuecore/issues).
