...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new cryptorescuecore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new cryptorescuecore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = cryptorescuecore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new cryptorescuecore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function cryptorescuecoreTest() {
      block1 = cryptorescuecore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('cryptorescuecore', cryptorescuecoreTest, {maxTime: maxTime});
